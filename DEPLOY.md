...
| 服务 | 镜像来源 | 端口 | 数据卷 |
|---|---|---|---|
| **ink-tavern**（主服务：剧本/存档/对话/设置） | `Dockerfile.ink-tavern`（多阶段，内嵌前端） | 8080 | `./data/server` |
| **cloud**（云端公共剧本库） | `Dockerfile.cloud`（纯 Go 标准库） | 8787 | `./data/cloud` |
...
- 打开 **设置 → 公共剧本库**，云端服务地址填 `http://localhost:8787`，保存后即可浏览/上传/下载/导入共享剧本。
...
- **端口被占用**：改 `docker-compose.yml` 里的 `ports` 映射（如 `8081:8080`），云端地址同步改。
